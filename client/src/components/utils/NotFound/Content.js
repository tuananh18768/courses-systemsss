...
import { fetchAllDepartment, dispatchGetAllDepartment, fetchOneDepartment, dispatchGetOneDepartment } from '../../../redux/actions/departmentAction'
...
export default function Content() {
    ...
    useEffect(() => {
        if (token) {
            ...
            fetchOneDepartment(chooseDepart,token).then(res => (dispatch(dispatchGetOneDepartment(res))))
        }
    ...
    const handleDownloadZip = async () => {
        try {
            const res = await axios.get('/api/downloadZip', { headers: { Authorization: token }, responseType: 'blob' })
    ...
    const handleDownloadCsv = async () => {
        try {
            const res = await axios.get('/api/downloadCsv', { headers: { Authorization: token }, responseType: 'blob' })
            console.log(res)
            const str = res.headers["content-disposition"];
            const startSlice = str.indexOf('"');
            const endSlice = str.lastIndexOf('"');
    ...
    return (
        <>
            ...
            <div className="content_main" style={{ display: 'flex' }}>
                ...
                <div className="wrapIdeas">
                    <div className="wrapIdeas__header" style={{ border: 0 }}>
                        ...
                        <div className="dropdown" style={{ display: 'flex', alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginRight: 10 }}>
                                <input style={radio} onClick={() => { setSort(true) }} type="radio" name="sort" />Decessding
                                <input style={radio} onClick={() => { setSort(false) }} type="radio" name="sort" checked={!sort} />Acessding
                            </div>
                            <select className=" btnVI" onChange={(e) => { setChooses(e.target.value) }}>
                    ...
                    {isManager && <div className="wrapIdeas__header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        {!departmentChange ? <div>
                            <button disabled style={{cursor: 'auto'}} id="downloadZip" onClick={handleDownloadZip} to="">Download all zip</button>
                            <button disabled style={{cursor: 'auto'}} id="downloadCsv" onClick={handleDownloadCsv} to="">Download all csv</button>
                        </div> : <div>
                            <button id="downloadZip" style={{cursor: 'pointer'}} onClick={handleDownloadZip} to="">Download all zip</button>
                            <button id="downloadCsv" style={{cursor: 'pointer'}} onClick={handleDownloadCsv} to="">Download all csv</button>
                        </div>
                        }

                        <select style={{ width: 300 }} className=" btnVI" name="changeDepart" onChange={(e)=>{
                        ...
                        {departmentAlls.map((current, index) => {
                            return <option key={index} value={current._id}>{current.name}</option>
                        })}
                        {/* <option value="likes">Most Like</option>
                                <option value="views">Most Views</option>
                                <option value="comments">Most Comments</option>
                ...
                <div className="wrapIdeas__content" style={{ minHeight: '650px' }}>
                    <IdeaStaff currentPost={currentPost} currentPostStaff={currentPostStaff} departmentChange={departmentChange}/>
                </div>
                <div>
                    <Pagination currentPage={currentPage} postPerpage={postPerpage} handleClickPage={handleClickPage} />
                </div>
            </div>
        </div>
        </>
